...
foreach ($sa in $storageAccounts) {
    ...
    Get-AzStorageTable -Context $saContext | where {$_.Name -match "^WADMetrics.*(\d\d\d\d\d\d\d\d)$" -and $Matches[1] -lt $removeDate} | Remove-AzStorageTable -Context $saContext
}